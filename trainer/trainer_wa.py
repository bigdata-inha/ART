...
class Trainer(trainer_warehouse.GenericTrainer):
    ...
    def train(self, epoch, triplet=False):
        ...
        for data, target, traindata_idx in tqdm(self.train_data_iterator):
            ...
            if tasknum > 0 and triplet == True:
                if epoch > self.args.triplet_epoch:
                    ...
                    for i in range(data.size()[0]):
                        ...
                        n_feature[i] = self.class_anchor[target_b[i]]

                    loss_triplet = self.triplet_loss(a_feature, p_feature, n_feature)
                    loss_triplet = loss_triplet * self.args.triplet_lam
                    ...
                    # loss_mr = self.mr_loss(a_feature[new_idx, :], n_feature, n_target) * self.args.triplet_lam
            ...
            if (self.args.KD == "naive_global") or (self.args.KD == "naive_local"):
